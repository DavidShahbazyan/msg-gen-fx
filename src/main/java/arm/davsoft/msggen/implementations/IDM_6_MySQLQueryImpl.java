...
public final class IDM_6_MySQLQueryImpl implements SQLQuery {
    ...
    private static final String LOAD_UNUSED_MESSAGE_IDS = "SELECT DISTINCT iMessageID FROM kb_am_Message WHERE iMessageID >= @@From AND iMessageID <= @@To AND (CAST(iMessageID AS CHAR(10)) = sText OR sText IS NULL OR sText = '')";
    ...
    private static final String LOAD_MESSAGES = "SELECT iMessageID, sText FROM kb_am_Message WHERE CAST(iMessageID AS CHAR(10)) != sText and iMessageID >= @@From and iMessageID <= @@To and iLanguageID = 1";
