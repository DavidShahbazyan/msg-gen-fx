...
public final class IDM_6_MySQLQueryImpl implements SQLQuery {
    ...
    private static final String BACKUP_MESSAGES_TABLE = "SELECT * INTO kb_am_Message" + tableSuffix + " FROM kb_am_Message";
...
//    private static final String REMOVE_UNUSED_MESSAGES = "UPDATE kb_am_Message SET sText = iMessageID, sComment = NULL, sDescription = 'Online DE', sUserName = NULL, iModuleID = NULL, iCategoryID = NULL, bIsGlobal = NULL WHERE iMessageID NOT IN (@@MessageIds) AND iMessageID >= @@RangeStart and iMessageID <= @@RangeEnd";
    private static final String LOAD_UNUSED_MESSAGE_IDS = "SELECT DISTINCT iMessageID FROM kb_am_Message WHERE iMessageID >= ? AND iMessageID <= ? AND (CAST(iMessageID AS CHAR(10)) = sText OR sText IS NULL OR sText = '')";
    ...
    private static final String LOAD_MESSAGES = "SELECT iMessageID, sText FROM kb_am_Message WHERE iMessageID >= ? AND iMessageID <= ? AND CAST(iMessageID AS CHAR(10)) != sText AND iLanguageID = 1";
    private static final String LOAD_MESSAGES_EXCEPT = "SELECT iMessageID, sText FROM kb_am_Message WHERE iMessageID >= ? AND iMessageID <= ? AND iMessageID NOT IN (?) AND CAST(iMessageID AS CHAR(10)) != sText AND iLanguageID = 1";
    ...
    private static final String CREATE_EMPTY_MESSAGES = "CALL DE_InsertMissedMessages (?, ?)";
