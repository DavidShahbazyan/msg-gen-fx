...
//    private static final String REMOVE_UNUSED_MESSAGES = "UPDATE DE_Messages SET MessageText = MessageID, IsGlobal = 0, WorkingCopyID = " + workingCopyId + " WHERE MessageID NOT IN (@@MessageIds) AND MessageID >= @@RangeStart and MessageID <= @@RangeEnd";
    private static final String LOAD_UNUSED_MESSAGE_IDS = "SELECT DISTINCT MessageID FROM DE_Messages WHERE MessageID >= ? AND MessageID <= ? AND (CAST(MessageID AS VARCHAR(10)) = MessageText OR MessageText IS NULL OR MessageText = '')";
...
//    private static final String LOAD_MESSAGES = "SELECT MessageID, MessageText FROM DE_Messages WHERE CAST(MessageID AS VARCHAR(10)) != MessageText AND MessageID >= @@RangeStart AND MessageID <= @@RangeEnd AND LanguageID = 1 AND WorkingCopyID = " + workingCopyId;
    private static final String LOAD_MESSAGES = "SELECT MessageID, MessageText FROM DE_Messages WHERE MessageID >= ? AND MessageID <= ? AND CAST(MessageID AS VARCHAR(10)) != MessageText AND WorkingCopyID = " + workingCopyId + " GROUP BY MessageID, MessageText";
    private static final String LOAD_MESSAGES_EXCEPT = "SELECT MessageID, MessageText FROM DE_Messages WHERE MessageID >= ? AND MessageID <= ? AND MessageID NOT IN (?) AND CAST(MessageID AS VARCHAR(10)) != MessageText AND WorkingCopyID = " + workingCopyId + " GROUP BY MessageID, MessageText";
