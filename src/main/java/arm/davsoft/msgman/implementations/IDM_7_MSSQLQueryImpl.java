...
//    private static final String LOAD_MESSAGES = "SELECT MessageID, MessageText FROM DE_Messages WHERE CAST(MessageID AS VARCHAR(10)) != MessageText AND MessageID >= @@RangeStart AND MessageID <= @@RangeEnd AND LanguageID = 1 AND WorkingCopyID = " + workingCopyId;
    ...
    private static final String LOAD_MESSAGES_EXCEPT = "SELECT MessageID, MessageText FROM DE_Messages WHERE CAST(MessageID AS VARCHAR(10)) != MessageText AND MessageID NOT IN (@@MessageIds) MessageID >= @@RangeStart AND MessageID <= @@RangeEnd AND WorkingCopyID = " + workingCopyId + " GROUP BY MessageID, MessageText";
