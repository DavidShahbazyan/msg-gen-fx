...
public final class IDM_7_MySQLQueryImpl implements SQLQuery {
    ...
    private static final String LOAD_UNUSED_MESSAGE_IDS = "SELECT DISTINCT MessageID FROM DE_Messages WHERE MessageID >= @@From AND MessageID <= @@To AND (CAST(MessageID AS CHAR(10)) = MessageText OR MessageText IS NULL OR MessageText = '')";
    ...
    private static final String LOAD_MESSAGES = "SELECT MessageID, MessageText FROM DE_Messages WHERE CAST(MessageID AS CHAR(10)) != MessageText AND MessageID >= @@From AND MessageID <= @@To AND LanguageID = 1 AND WorkingCopyID = " + workingCopyId;
